...
protected:
  static std::size_t GetIndex(const uint16_t address) { return address - GetAddressStart(); }
 ...
 public:
  ...
  void GetRegisters(const std::size_t address, const std::size_t register_count, ArrayView<uint8_t>* data_view) {
  ...
  //  void set_registers_callback(std::size_t, std::size_t, const ArrayView<const uint8_t>&) {}
...
inline bool is_true(const bool value) {
  return value;
}
...
class UpdatingRegisterDataStore : public RegisterDataStore {
 ...
 public:
  UpdatingRegisterDataStore(uint16_t* data, const size_t length, bool* flags) :
    ...
    void set_registers_callback(std::size_t address, std::size_t register_count, const ArrayView<const uint8_t>&) {
